...
export const PropertyGallery: React.FC<PropertyGalleryProps> = ({
  ...
  /**
   * Instagram's embed page is a whole post — profile header, media, then the
   * likes and comment bar — so its height depends on the reel and on how wide
   * we render it. It reports that height to the parent with the same MEASURE
   * message Instagram's own embed.js sizes iframes from. Matching the frame to
   * it is what keeps the reel from being cropped to a sliver on a phone; until
   * it arrives (or if it never does) the frame falls back to a portrait box.
   */
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      ...
      if (event.source !== instagramIframeRef.current?.contentWindow) return;

      let payload = event.data;
      if (typeof payload === 'string') {
        try { payload = JSON.parse(payload); } catch { return; }
      }
      if (payload?.type !== 'MEASURE') return;

      const height = Number(payload?.details?.height);
      if (Number.isFinite(height) && height > 0) setInstagramHeight(height);
    };
  ...
  return (
    ...
    <div className="overflow-hidden shadow-sm border-b border-neutral-200" data-no-swipe-back>
      {/* ── Gallery frame — 4:3 on a phone, 16:9 from `sm` up (Zillow standard).
           ...
           Video keeps its authored shape at every width: the embeds are 16:9, or 9:16
          ...
          <div className="relative w-full h-full bg-black flex items-center justify-center overflow-hidden">
            ...
            {hasGeneratedVideo ? (
              ...
              <>
                ...
                {/* Platform badge. Instagram's embed opens with its own profile
                    header, which this would sit on top of, and it already carries
                    the platform's branding — so it is left off there. */}
