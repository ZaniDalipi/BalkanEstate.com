...
const InstagramEmbed: React.FC<{ url: string }> = ({ url }) => {
  ...
  // Re-process embeds when container is ready
  useEffect(() => {
    if (containerRef.current && postId) {
...
// TikTok Embed Component using official blockquote method
      ...
      <div className="relative w-full h-[200px] xs:h-[250px] sm:h-[350px] md:h-[400px] lg:h-[450px] landscape:h-[50vh] landscape:min-h-[200px] bg-neutral-200">
        {viewMode === 'photos' ? (
          ...
          isVerticalVideoPlatform ? (
            // TikTok/Instagram/Facebook - Official embed using blockquote method
            <div className="relative w-full h-full overflow-hidden">
              {/* Blurred background for HDR effect */}
              <div
              ...
              <div className="relative z-10 w-full h-full">
                {videoPlatform === 'instagram' && (
                  <InstagramEmbed url={property.tourUrl || ''} />
                )}
                {videoPlatform === 'tiktok' && (
                  <TikTokEmbed url={property.tourUrl || ''} />
                )}
                {videoPlatform === 'facebook' && (
                  <div className="w-full h-full flex items-center justify-center">
                    <iframe
                      src={`https://www.facebook.com/plugins/video.php?href=${encodeURIComponent(property.tourUrl || '')}&show_text=false&autoplay=true`}
