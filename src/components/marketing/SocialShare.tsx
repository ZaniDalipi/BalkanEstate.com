...
const sharePlatforms: Record<string, SharePlatform> = {
  ...
  email: {
    ...
    getUrl: (url, title, description) => `mailto:?subject=${encodeURIComponent(title)}&body=${encodeURIComponent(`${description || title}\n\n${url}`)}`,
  },
};
          ...
          <div className="absolute right-0 mt-2 w-48 bg-white rounded-xl shadow-lg border border-neutral-200 py-2 z-50">
            {platforms.map((platform) => (
              <button
                ...
                className="w-full flex items-center gap-3 px-4 py-2 hover:bg-neutral-50 transition-colors"
              >
                {platform === 'copy' ? (
                  <>
                    <svg className="w-5 h-5 text-neutral-600" fill="none" stroke="currentColor" viewBox="0 0 24 24">
