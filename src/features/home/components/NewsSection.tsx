    ...
    >
      ...
      <div className={`h-36 bg-gradient-to-br ${item.imageGradient} relative overflow-hidden`}>
        ...
        <div className="absolute top-3 left-3 flex items-center gap-1.5 bg-white/90 backdrop-blur-sm rounded-full px-2.5 py-1">
          <span className="text-xs font-semibold text-slate-800">{item.countryCode}</span>
          <span className="text-[10px] text-slate-500">{item.country}</span>
        </div>
        {/* Category tag */}
        <span className={`absolute top-3 right-3 px-2 py-0.5 rounded-full text-[9px] font-bold uppercase tracking-wider ${CATEGORY_COLORS[item.category] || 'bg-neutral-100 text-neutral-700'}`}>
          {item.category}
        </span>
      </div>
      ...
      <div className="p-4">
        ...
        <span className="inline-flex items-center gap-1 mt-2.5 text-[11px] text-blue-500 font-medium opacity-0 group-hover:opacity-100 transition-opacity">
          Read Original <span>&rarr;</span>
        </span>
      </div>
    </motion.a>
  );
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {news.length === 0 && (
          <div className="text-center py-12 text-slate-400 text-sm">
            No news available for this country yet.
          </div>
        )}
      </div>
    </section>
