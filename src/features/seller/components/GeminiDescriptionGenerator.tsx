...
const GeminiDescriptionGenerator: React.FC<{ propertyToEdit: Property | null }> = ({ propertyToEdit }) => {
    ...
    // Some listings (e.g. imported from an external feed) can be missing fields the
    // create form always fills in. Surface what's missing so the user knows to fix it.
    const missingFields = propertyToEdit ? [
        !propertyToEdit.address && t('seller:createListing.location.address', 'Address'),
        !propertyToEdit.city && t('seller:createListing.location.city', 'City'),
        !propertyToEdit.country && t('seller:createListing.location.country', 'Country'),
        !propertyToEdit.description && t('seller:createListing.fields.description', 'Description'),
        (!propertyToEdit.images || propertyToEdit.images.length === 0) && t('seller:createListing.upload.uploadProperty', 'Photos'),
    ...
    return (
        ...
        {missingFields.length > 0 && (
            <div className="mb-6 flex items-start gap-3 rounded-xl border border-amber-300 bg-amber-50 p-4">
                ...
                <div>
                    <p className="text-sm font-semibold text-amber-800">{t('seller:createListing.missingFields.title', 'This listing is missing some details')}</p>
                    <p className="text-sm text-amber-700 mt-1">
                        {t('seller:createListing.missingFields.description', 'Please fill in the following before saving: {{fields}}', { fields: missingFields.join(', ') })}
                    </p>
                </div>
            </div>
        )}
