...
const BusinessCard: React.FC<BusinessCardProps> = ({ listing, onClick, onQuoteRequest }) => {
  ...
  const handleQuickWhatsApp = (e: React.MouseEvent) => {
    e.stopPropagation();
    if (listing.whatsapp) window.open(`https://wa.me/${listing.whatsapp.replace(/[^0-9+]/g, '')}`, '_blank', 'noopener,noreferrer');
  };
          ...
          <div className="absolute bottom-2 right-3 flex items-center gap-1.5 translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300 ease-out z-10">
            ...
            {listing.whatsapp && (
              <button
                ...
                className="w-8 h-8 rounded-full bg-green-500/30 backdrop-blur-md border border-green-300/30 flex items-center justify-center hover:bg-green-500/50 hover:scale-110 active:scale-90 transition-all"
                aria-label="WhatsApp"
                title="WhatsApp"
              >
