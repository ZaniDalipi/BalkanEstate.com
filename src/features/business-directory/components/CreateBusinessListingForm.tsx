...
import { BUSINESS_CATEGORIES, type BusinessCategory, type CreateBusinessListingData, type ListingType, PRICE_RANGES, PAYMENT_METHODS, BALKAN_LANGUAGES, type PriceRange, type PaymentMethod } from '@/src/shared/types/businessListing.types';
...
const CreateBusinessListingForm: React.FC<CreateBusinessListingFormProps> = ({ onBack, onSuccess }) => {
  ...
  const [phoneCountryCode, setPhoneCountryCode] = useState('+383');
  const [phoneDigits, setPhoneDigits] = useState('');
  const [whatsappCountryCode, setWhatsappCountryCode] = useState('+383');
  const [whatsappDigits, setWhatsappDigits] = useState('');
  const [viberCountryCode, setViberCountryCode] = useState('+383');
  const [viberDigits, setViberDigits] = useState('');
  ...
  const handleSubmit = useCallback(async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (whatsappClean) cleanData.whatsapp = `${whatsappCountryCode}${whatsappClean}`;
      if (viberClean) cleanData.viber = `${viberCountryCode}${viberClean}`;
      if (formData.languages && formData.languages.length > 0) cleanData.languages = formData.languages;
      if (formData.yearEstablished) cleanData.yearEstablished = formData.yearEstablished;
      if (formData.licenseNumber?.trim()) cleanData.licenseNumber = formData.licenseNumber.trim();
      if (formData.serviceAreas && formData.serviceAreas.length > 0) cleanData.serviceAreas = formData.serviceAreas;
      if (formData.priceRange) cleanData.priceRange = formData.priceRange;
      if (formData.paymentMethods && formData.paymentMethods.length > 0) cleanData.paymentMethods = formData.paymentMethods;

      const social = formData.socialMedia;
      if (social?.facebook || social?.instagram || social?.linkedin || social?.tiktok) {
          ...
          <Animated variant="fadeInUp" delay={100}>
            <div className="bg-white rounded-2xl border border-neutral-200 p-6 shadow-sm space-y-4">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label htmlFor="viber" className="block text-sm font-medium text-neutral-700 mb-1">
          ...
          <Animated variant="fadeInUp" delay={200}>
            <div className="bg-white rounded-2xl border border-neutral-200 p-6 shadow-sm space-y-4">
              ...
              <div className="space-y-3">
                ...
                <div>
                  <label htmlFor="tiktok" className="block text-sm font-medium text-neutral-700 mb-1">TikTok</label>
                  <input
                    id="tiktok"
                    type="url"
