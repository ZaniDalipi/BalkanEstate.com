...
const VillaFilters: React.FC<VillaFiltersProps> = ({
    ...
    const chipActive   = 'bg-secondary/10 text-primary-dark border-secondary font-semibold';
    const chipInactive = 'bg-white/60 text-gray-500 border-gray-200 hover:border-secondary/50 hover:text-primary';
    ...
    if (compact) {
        return (
            <div className="px-3 py-2 space-y-2">
                ...
                {/* Row 2 — Price per night */}
                <div className="grid grid-cols-2 gap-2">
                    <div>
                        <label className={labelClasses}>{t('villas:filters.minRentPerNight', 'Min €/night')}</label>
                    ...
                    <div>
                        <label className={labelClasses}>{t('villas:filters.maxRentPerNight', 'Max €/night')}</label>
                        <div className="relative">
                            <span className="absolute left-1.5 top-1/2 -translate-y-1/2 text-[11px] text-gray-400">{currencySymbol}</span>
                            <input
                                type="number"
                ...
                {/* Row 4 — Furnishing + action buttons */}
                <div className="flex items-center justify-between gap-2">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                        <button
                            onClick={onReset}
                            className="text-[11px] text-gray-400 hover:text-gray-600 py-1.5 transition-colors whitespace-nowrap"
                        >
                        ...
                        <Button
                            ...
                            size="sm"
                            onClick={onSearch}
                            className="text-xs font-semibold whitespace-nowrap rounded-xl"
                        >
        ...
        <div className="space-y-4 p-4">
            ...
            <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wide">
                    {t('villas:filters.viewType', 'Setting')}
                </label>
            ...
            <div>
                ...
                <div className="grid grid-cols-2 gap-3">
                    <div>
                        <label className="block text-[11px] text-gray-400 mb-0.5">{t('villas:filters.minRentPerNight', 'Min €/night')}</label>
                        <div className="relative">
                    ...
                    <div>
                        <label className="block text-[11px] text-gray-400 mb-0.5">{t('villas:filters.maxRentPerNight', 'Max €/night')}</label>
                        <div className="relative">
                            <span className="absolute left-2.5 top-1/2 -translate-y-1/2 text-xs text-gray-400">{currencySymbol}</span>
                            <input
                                type="number"
            ...
            <div>
                <label className="block text-xs font-medium text-gray-400 mb-1.5 uppercase tracking-wide">
                    {t('villas:filters.amenities', 'Premium Amenities')}
                </label>
                <div className="flex flex-wrap gap-2">
                    {([
                        { key: 'hasPool'   as keyof Filters, label: t('villas:filters.pool',   'Pool'),   emoji: '🏊' },
            ...
            <div className="flex gap-2 pt-1">
                ...
                <button
                    onClick={onReset}
                    className="text-sm text-gray-400 hover:text-gray-600 py-2 px-3 transition-colors"
                >
                    {t('villas:filters.reset', 'Reset')}
                </button>
            </div>
