...
// User listing type for listing promotion
...
const PricingPage: React.FC = () => {
  ...
  const promotionPricing = {
    featured: { 7: 1.99, 30: 4.99, 90: 9.99 },
    highlight: { 7: 3.99, 30: 9.99, 90: 19.99 },
    premium: { 7: 7.99, 30: 19.99, 90: 39.99 },
  };

  // Agency feature pricing
  const agencyFeaturePricing = {
    spotlight: { price: 29.99, duration: '30 days' },
    homepage: { price: 49.99, duration: '30 days' },
    premium: { price: 99.99, duration: '30 days' },
  };

  // Fetch products from API
  useEffect(() => {
    const fetchProducts = async () => {
      // Only fetch products for seller/buyer tabs
      if (activeTab !== 'seller' && activeTab !== 'buyer') {
        setLoading(false);
        return;
      }
      setLoading(true);
  ...
  // Fetch user listings when listing tab is active
  useEffect(() => {
    const fetchUserListings = async () => {
      if (activeTab !== 'listing' || !state.isAuthenticated || !state.currentUser) {
        setUserListings([]);
        return;
      }
    ...
    fetchUserListings();
  }, [activeTab, state.isAuthenticated, state.currentUser]);

  // Handle listing promotion selection
  ...
  // Handle listing selection for promotion
  ...
  const handlePurchasePromotion = async () => {
    ...
    const price = promotionPricing[selectedPromoTier][selectedDuration];
    setSelectedPlan({
      name: `${selectedPromoTier.charAt(0).toUpperCase() + selectedPromoTier.slice(1)} Promotion - ${selectedDuration} days`,
      price,
      interval: 'month',
      productId: `listing_${selectedPromoTier}_${selectedDuration}d`,
    });
    setShowPaymentWindow(true);
  };

  // Handle agency feature purchase
  const handleAgencyFeature = (tier: 'spotlight' | 'homepage' | 'premium') => {
    ...
    if (!state.currentUser?.agencyId) {
      dispatch({
        ...
        payload: {
          type: 'warning',
          title: t('pricing:agency.noAgency', 'No Agency Found'),
          message: t('pricing:agency.createAgencyFirst', 'You need to create an agency first before featuring it. Subscribe to Enterprise plan to create your agency.'),
        },
      });
      setActiveTab('seller');
      return;
    ...
    const pricing = agencyFeaturePricing[tier];
    setSelectedPlan({
      name: `Agency ${tier.charAt(0).toUpperCase() + tier.slice(1)} Feature`,
      price: pricing.price,
      interval: 'month',
      productId: `agency_feature_${tier}`,
    });
    setShowPaymentWindow(true);
  };
