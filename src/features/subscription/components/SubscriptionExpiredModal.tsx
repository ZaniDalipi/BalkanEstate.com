...
const SubscriptionExpiredModal: React.FC = () => {
  ...
  // Render the PaymentWindow even when the expired modal is hidden (after clicking Reactivate)
  const paymentWindow = (
    <PaymentWindow
      isOpen={showPaymentWindow}
      onClose={handlePaymentWindowClose}
      planName={paymentProps.planName}
      planPrice={paymentProps.planPrice}
      planInterval={paymentProps.planInterval}
      productId={paymentProps.productId}
      userRole={paymentProps.userRole}
      userEmail={paymentProps.userEmail}
      userCountry={paymentProps.userCountry}
      onSuccess={handlePaymentSuccess}
      onError={() => {
        // Payment error is displayed inside PaymentWindow itself; no extra action needed
      }}
    />
  );

  if (!isVisible) return paymentWindow;
