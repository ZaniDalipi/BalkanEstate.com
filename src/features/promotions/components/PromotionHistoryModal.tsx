          ...
          <div className="bg-gradient-to-r from-primary to-primary-dark p-6 text-white">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-xl font-bold">{t('seller:promotionHistory.title', 'Promotion History')}</h2>
              ...
              <>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
                  <div className="bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl p-4 text-center">
                    ...
                    <div className="text-xs text-blue-600/70">{t('seller:promotionHistory.promotions', 'Promotions')}</div>
                  </div>
                  <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-4 text-center">
                    <div className="text-2xl font-bold text-green-600">{totals.totalDaysPromoted}</div>
                    <div className="text-xs text-green-600/70">{t('seller:promotionHistory.daysPromoted', 'Days Promoted')}</div>
                  </div>
                  <div className="bg-gradient-to-br from-purple-50 to-pink-50 rounded-xl p-4 text-center">
                    <div className="text-2xl font-bold text-purple-600">{totals.totalViews}</div>
                    <div className="text-xs text-purple-600/70">{t('seller:promotionHistory.viewsGenerated', 'Views Generated')}</div>
                  </div>
                  <div className="bg-gradient-to-br from-amber-50 to-orange-50 rounded-xl p-4 text-center">
                    ...
                    <div className="text-xs text-amber-600/70">{t('seller:promotionHistory.inquiries', 'Inquiries')}</div>
                  </div>
                </div>

                {/* Total Spent */}
                <div className="bg-neutral-50 rounded-xl p-4 mb-6 flex items-center justify-between">
                  <span className="text-neutral-600 font-medium">{t('seller:promotionHistory.totalSpent', 'Total Spent on Promotions')}</span>
                ...
                {history.length === 0 ? (
                  <div className="text-center py-8 text-neutral-500">
                    <p>{t('seller:promotionHistory.noHistoryFound', 'No promotion history found for this property.')}</p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <h3 className="font-semibold text-neutral-700 mb-2">{t('seller:promotionHistory.promotionTimeline', 'Promotion Timeline')}</h3>
                        ...
                        >
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              {item.hasUrgentBadge && (
                                <span className="bg-red-100 text-red-700 text-xs font-semibold px-2 py-1 rounded-full">
                                  🔥 {t('seller:promotionHistory.urgentBadge', 'Urgent')}
                                </span>
                              )}
                              {item.isActive && (
                                <span className="bg-green-100 text-green-700 text-xs font-semibold px-2 py-1 rounded-full">
                                  {t('seller:promotionHistory.activeStatus', 'Active')}
                                </span>
                              )}
                              {item.isFromAgencyAllocation && (
                                <span className="bg-blue-100 text-blue-700 text-xs font-semibold px-2 py-1 rounded-full">
                                  {t('seller:promotionHistory.agencyBadge', 'Agency')}
                                </span>
                              )}
                            </div>
                            <div className="text-right">
                              <div className="font-bold text-neutral-800">
                                {item.price === 0 ? t('seller:promotionHistory.freeLabel', 'Free') : `€${item.price.toFixed(2)}`}
                              </div>
                              <div className="text-xs text-neutral-500">{item.duration} {t('seller:promotionHistory.daysLabel', 'days')}</div>
                            </div>
                          </div>
                          ...
                          <div className="flex items-center gap-4 text-sm text-neutral-600 mb-3">
                            ...
                            {item.isActive && (
                              <span className="text-green-600 font-medium">
                                {t('seller:promotionHistory.daysLeft', '{{count}} days left', { count: item.daysRemaining })}
                              </span>
                            )}
                          </div>
                          ...
                          <div className="flex items-center gap-4 text-sm">
                            <div className="flex items-center gap-1 text-blue-600">
                              ...
                              <span>{t('seller:promotionHistory.viewsLabel', '{{count}} views', { count: item.performance.views })}</span>
                            </div>
                            <div className="flex items-center gap-1 text-green-600">
                              <InquiriesIcon className="w-4 h-4" />
                              <span>{t('seller:promotionHistory.inquiriesLabel', '{{count}} inquiries', { count: item.performance.inquiries })}</span>
                            </div>
                            <div className="flex items-center gap-1 text-amber-600">
                              <HeartIcon className="w-4 h-4" />
                              <span>{t('seller:promotionHistory.savesLabel', '{{count}} saves', { count: item.performance.saves })}</span>
                            </div>
                          </div>
                          ...
                          {item.autoExtend && (
                            <div className="mt-2 flex items-center gap-1 text-xs text-indigo-600">
                              <span>🔄</span>
                              <span>{t('seller:promotionHistory.autoExtendEnabled', 'Auto-extend enabled')}</span>
                            </div>
                          )}
                        </div>
                      );
