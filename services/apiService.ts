 ...
 * If absent (first visit, expired, cleared), one GET to /health
...
const ensureCsrfToken = (): Promise<void> => {
  ...
  if (!_csrfBootstrap) {
    _csrfBootstrap = fetch(`${API_URL}/health`, { credentials: 'include' })
      .catch(() => {/* ignore — worst case CSRF header is absent */})
...
function transformBackendProperty(backendProp: any): Property {
  ...
  return {
    ...
    isNegotiable: backendProp.isNegotiable || false,
    ...
    kitchens: backendProp.kitchens,
    diningRooms: backendProp.diningRooms,
    toilets: backendProp.toilets,
...
function transformToBackendProperty(frontendProp: Property): any {
  const result: any = {
    ...
    isNegotiable: frontendProp.isNegotiable || false,
    ...
    kitchens: frontendProp.kitchens,
    diningRooms: frontendProp.diningRooms,
    toilets: frontendProp.toilets,
