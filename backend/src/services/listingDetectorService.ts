 ...
 * Only call this on raw text a human pasted — anything already parsed (or
 * generated by our own code, e.g. CSV/Excel rows) should go through
 * `detectFromParsedListingData` instead, which skips this step entirely.
 */
...
export const detectFromJsonSample = (jsonString: string): DetectResult => {
  ...
  return detectFromParsedListingData(parsed);
};

/**
 * Same listing-shape detection as `detectFromJsonSample`, but takes an
 * already-parsed JS value (e.g. an array of row objects from a CSV/Excel
 * upload) instead of raw text. There is no JSON.parse or text cleanup here
 * at all, so there's no risk of the human-paste cleanup (or any parser)
 * corrupting values that happen to contain quotes, commas, or other
 * punctuation — the data is used exactly as the caller already parsed it.
 */
export const detectFromParsedListingData = (parsedInput: unknown): DetectResult => {
  let parsed = unwrapMongoExtendedJson(parsedInput);
