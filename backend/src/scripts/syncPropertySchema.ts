...
const log = scriptLogger.child('SyncPropertySchema');
...
const run = async () => {
  ...
  log.info(`Connecting to ${process.env.NODE_ENV || 'development'} database...`);
  ...
  log.info(`Done! ${commonUpdated} properties updated, ${rentalUpdated} rental listings updated.`);
