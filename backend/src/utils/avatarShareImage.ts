/**
 * Generated (DiceBear) avatars as shareable images — backend copy.
 *
 * A byte-for-byte mirror of config/avatarShareImage.ts. It is duplicated only
 * because the backend is its own package (`tsconfig.rootDir` is `backend/src`)
 * and cannot import from the frontend tree. `src/tests/og-profile-share.test.ts`
 * asserts both copies produce identical URLs, so drift fails the build rather
 * than quietly showing a different face on a shared link.
 *
