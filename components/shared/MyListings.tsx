...
const ListingCard: React.FC<{
    ...
    // Feed-imported listings can arrive missing fields the manual form always fills in.
    // Flag those so the seller knows to open Edit and complete them.
                ...
                <div onClick={handleCardClick} className="cursor-pointer">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                        ...
                        {missingFieldsCount > 0 && (
                            <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium bg-amber-100 text-amber-700" title={t('seller:myListings.incompleteHint', 'Open Edit to fill in the missing details.')}>
                                <svg className="w-3 h-3" fill="currentColor" viewBox="0 0 20 20">
                                    <path fillRule="evenodd" d="M8.257 3.099c.765-1.36 2.722-1.36 3.486 0l5.58 9.92c.75 1.334-.213 2.98-1.742 2.98H4.42c-1.53 0-2.493-1.646-1.743-2.98l5.58-9.92zM11 13a1 1 0 11-2 0 1 1 0 012 0zm-1-8a1 1 0 00-1 1v3a1 1 0 002 0V6a1 1 0 00-1-1z" clipRule="evenodd"/>
                                </svg>
                                {t('seller:myListings.incomplete', 'Incomplete - Edit to fix')}
                            </span>
                        )}
                    </div>
                    {property.propertyId && (
...
const MyListings: React.FC<{ sellerId: string }> = ({ sellerId }) => {
    ...
    const filteredAndSortedProperties = useMemo(() => {
        ...
        // Apply search (property ID, title, address, city)
        ...
        <div className="space-y-6">
            ...
            <div className="relative max-w-sm">
                <input
                    ...
                    placeholder={t('seller:myListings.searchListings', 'Search by ID, title, address...')}
                    ...
                    aria-label={t('seller:myListings.searchListings', 'Search listings')}
                />
